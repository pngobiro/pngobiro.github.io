#!/usr/bin/env python3
"""
从 KSL 仓库下载所有可用的 ATP 过去试卷 - 完整扫描版本
"""
...
def main():
    print("=" * 80)
    print("从 KSL 仓库下载所有可用的 ATP 过去试卷")
    print("=" * 80)
    print()
    
    # Scan larger handle ranges
    handle_ranges = [
        (1, 100),
        (100, 200),
        (200, 300),
    ...
    for start, end in handle_ranges:
        ...
        print(f"  在此范围找到 {found_in_range} 个试卷，总共 {len(all_papers)} 个")
        print()
    ...
    for paper in all_papers:
        key = (paper['subject'], paper['year'], paper['session'])
        if key not in seen:
            seen.add(key)
            unique_papers.append(paper)
    
    print(f"去重后: {len(unique_papers)} 个唯一试卷")
    print()
    ...
    for i, paper in enumerate(unique_papers, 1):
        print(f"[{i}/{len(unique_papers)}] {paper['filename'][:70]}...")
