...
class ExamManager:
    ...
    def load_config(self):
        """Loads configuration from exam-manager-config.json."""
        config_file = self.root_dir / "exam-manager-config.json"
        defaults = {
            "program_name": "Educational Content",
            "program_short_name": "EDU",
            "program_full_name": "Educational Content Archive",
            "excluded_dirs": ["assets", "exams-manager"]
        }
        if config_file.exists():
            try:
                ...
                self.log(f"Warning: Could not load config: {e}", Colors.WARNING)
        return defaults
    ...
    def scan_for_subjects(self):
        ...
        excluded.update(['KMTC', 'quizzes', 'starter_kit_temp'])
...
"""
    ...
    def generate_html_index(self, force=False):
        ...
        short_name = self.config.get('program_short_name', 'EDU')
        full_name = self.config.get('program_full_name', 'Educational Content Archive')
        
        if main_index.exists() and not force:
            self.log("  Root index already exists. Skipping root generation.", Colors.BLUE)
        else:
            html_content = [
                self.generate_html_head(f"Educational Content Archive | {short_name}", assets_path),
                "<body>",
                "    <button class='mobile-nav-toggle' aria-label='Toggle Navigation'><i class='fas fa-bars'></i></button>",
                "    <div class='sidebar-backdrop'></div>",
                "    <div class='layout-container'>",
                "        <aside class='sidebar'>",
                f"            <div class='sidebar-header'><div class='logo'><i class='fas fa-graduation-cap'></i> <span>{short_name}</span></div></div>",
                "            <nav class='sidebar-nav'>",
            ...
            html_content.extend(["</div></section></div>", f"<footer><p><strong>Educational Content Archive</strong></p><p>{full_name} Professional Education Management</p></footer>", "</main></div>", f"<script src='{assets_path}/js/sidebar-navigation.js'></script>", "</body></html>"])
