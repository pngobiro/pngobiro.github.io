# Converting Past Papers from PDF to HTML: Detailed Instructions (v2)

This guide provides detailed instructions on converting KNEC past papers from PDF to HTML, ensuring consistency and proper formatting. **Version 2 incorporates lessons learned during the conversion process.**
...
```
pastpapers/
├── index.html                  # Main landing page listing all papers
...
├── examples/                    # Contains HTML examples and this guide
...
│   └── pdf_to_html_instructions.md # This file
├── [paper-name].html         # Individual past paper HTML files (e.g., 2023j.html)
├── [paper-name]_ocr.txt      # OCR output text files (e.g., 2023j_ocr.txt)
└── [paper-name].pdf          # Original PDF files (e.g., 2023j.pdf)
```

**Important Notes:**
*   The sidebar (`components/sidebar.html`) is embedded directly into `index.html` and each individual paper's HTML file.
*   `sidebar-include.js` and `template.html` are no longer actively used but kept for reference.
...
10. **Final Review:** Verify structure, content, and links across all generated files. Use `open index.html` in the correct project directory.
